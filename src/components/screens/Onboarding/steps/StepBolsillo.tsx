        ...
        <div className="budget-display">
          <div className="budget-amount">
            {profile.presupuesto === 0
              ? 'Solo pública'
              : `$${(profile.presupuesto / 1e6).toFixed(1)}M / sem`}
          </div>
        ...
        <div className="field">
          ...
          <div className="chips">
            {['Sí, claro', 'Solo si es necesario', 'No'].map((r) => (
              <Chip
                key={r}
                active={profile.icetex === r}
                onClick={() => setProfile((p) => ({ ...p, icetex: r }))}
              >
                {r}
              </Chip>
            ))}
        ...
        <div className="field">
          ...
          <div className="chips">
            {['Sí, desde el inicio', 'A partir de 3er semestre', 'No, solo estudiar'].map((r) => (
              <Chip
                key={r}
                active={profile.trabajar === r}
                onClick={() => setProfile((p) => ({ ...p, trabajar: r }))}
              >
                {r}
              </Chip>
            ))}
