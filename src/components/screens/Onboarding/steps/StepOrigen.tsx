  ...
  return (
    <div className="grid-2">
      <div className="field">
        ...
        <select
          ...
          onChange={(e) => {
            ...
            setProfile((p) => ({ ...p, ciudad: e.target.value, regionId: c?.regionId ?? p.regionId }));
          }}
        >
          {CIUDADES.map((c) => (
      ...
      <div className="field">
        ...
        <div className="chips">
          {[1,2,3,4,5,6].map((n) => (
            <Chip key={n} active={profile.estrato === n} onClick={() => setProfile((p) => ({ ...p, estrato: n }))}>
              Estrato {n}
            </Chip>
          ))}
        </div>
      ...
      <div className="field span-2">
        ...
        <div className="chips">
          {['Grado 10','Grado 11','Bachiller graduado','Estudiante universitario (cambio)','Otro'].map((g) => (
            <Chip key={g} active={profile.grado === g} onClick={() => setProfile((p) => ({ ...p, grado: g }))}>{g}</Chip>
          ))}
        </div>
      </div>
    </div>
