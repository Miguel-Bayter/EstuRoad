...
export function Compare() {
  ...
  const ROWS: { label: string; render: (c: Carrera) => React.ReactNode }[] = [
    { label: 'Match', render: (c) => <span className={`compare-score ${(c.score ?? 0) > 80 ? 'compare-score--high' : (c.score ?? 0) > 65 ? 'compare-score--mid' : 'compare-score--low'}`}>{c.score}</span> },
    { label: 'Tipo', render: (c) => c.tipo },
    { label: 'Duración', render: (c) => c.duracion },
    { label: 'Salario entrada', render: (c) => <span className="compare-value--sm">{formatCOP(c.salarioEntrada)}</span> },
    { label: 'Salario medio', render: (c) => <span className="compare-value--md">{formatCOP(c.salarioMedio)}</span> },
    ...
    { label: 'Proyección 2030', render: (c) => <span className="compare-projection">{c.proyeccion2030}</span> },
    ...
    { label: '¿Conviene quedarse?', render: (c) => (c.demandaPorRegion[profile.regionId as keyof typeof c.demandaPorRegion] ?? 0) >= 75 ? <span className="text-green">Sí, alta demanda local</span> : <span className="text-terra">Mejor migrar</span> },
  ];
